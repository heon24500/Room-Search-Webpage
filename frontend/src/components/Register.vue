              ...
              <div class="modal-container p-4">
                  <div class="h4 modal-header d-flex flex-column align-items-center">
                      ...
                      <div class="w-100 mt-4">
                          ...
                          <b-form-input class="mt-3 p-4" placeholder="아이디" v-model="email" type="email"></b-form-input>
                          <b-form-input class="mt-3 p-4" placeholder="비밀번호" v-model="password" type="password"></b-form-input>
                      </div>
        ...
        async signup() {
            ...
            try {
                if (this.email && this.name && this.password) {
                    if (this.seller) {
                        const {data} = await sellerApi.registerSeller(this.email, this.password, this.name);
                        console.log(data);
                        if (data.signup) {
                            alert("회원가입에 성공하였습니다.");
                            return;
                        ...
                        const {data} = await userApi.registerUser(this.email, this.password, this.name);
                        if (data.signup) {
                            alert("회원가입에 성공하였습니다.");
                            return;
                        }
